...
context("Tree depth at nodes is correctly reported") {
  SEXP values[] = {
      NumericVector::create(0),
      NumericVector::create(1),
      NumericVector::create(2),
    ...
    test_that("Depth is correctly computed for simple tree") {
      expect_true(gti.find_node(values[0])->parents_above() == 0);
    }
    ...
    test_that("Depth is correctly computed for simple tree") {
      expect_true(gti.find_node(values[5])->parents_above() == 3);
    }
}
...
context("is_last_sibling works") {
  ...
  expect_true(gti.find_node(values[4])->is_last_sibling());
}
#endif
