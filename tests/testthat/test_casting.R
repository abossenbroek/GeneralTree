...
test_that("Cast singelton tree to data frame", {
  ...
  result <- data.frame(key = 1, data = "parent1", parent_key = NA,
...
test_that("Cast more complex tree", {
  ...
  key <- c("root", "child1", "ten", "eleven", "twelve", "child2", "child4",
  ...
  parent_key <- c(NA, "root", "child1", "child1", "child1", "root", "child2", "child2",
              "child5", "child5", "child5", "child2", "root")


  result <- data.frame(key = key, data = data, parent_key = parent_key, stringsAsFactors = FALSE)
...
test_that("Cast singelton tree from data frame to tree", {
  ...
  test_tree <- data.frame(key = "root", data = "parent1", parent_key = NA, stringsAsFactors = FALSE)


  expect_equal(as.GeneralTree(test_tree), tree)
})
...
test_that("Cast more complex tree from data frame to tree", {
  ...
  test_tree <- data.frame(
    key = c("root", "child1", "child2"),
    data = c("parent1", "data1.1", "data1.2"),
    parent_key = c(NA, "root", "root"), stringsAsFactors = FALSE)
...
test_that("Cast more complex tree from data frame to tree", {
  ...
  test_tree <- data.frame(
    key = c("root", "child1", "child2", "child3"),
    data = c("parent1", "data3.1", "data1.2", "data1.3"),
    parent_key = c(NA, "child3", "root", "root"), stringsAsFactors = FALSE)
...
test_that("Cast data frame with non standard column names to tree", {
  ...
  test_tree <- as.GeneralTree(test_tree_df, key = "ID", data = "DATA",
                              parent_key = "PARENT")

  expect_equal(test_tree, tree)
})
...
test_that("Test for appropriate errors and warnings", {
  ...
  expect_error(as.GeneralTree(test_tree_df, key = "ID", parent_node = "PARENT"))
  expect_error(as.GeneralTree(test_tree_df, key = "ID", data = "DATA"))
  ...
  expect_error(as.GeneralTree(test_tree_df, key = "ID", data = "DATA", parent_key = "PARENT"))
  ...
  expect_warning(as.GeneralTree(test_tree_df, key = "ID", data = "DATA", parent_key = "PARENT"))
})
#
